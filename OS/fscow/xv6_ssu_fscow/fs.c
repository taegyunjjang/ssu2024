...
static uint
balloc(uint dev)  // allocate block
{
  int b, bi, m;
  struct buf *bp;
...
uint get_block(struct inode *ip, uint block) {
  uint new_block = balloc(ip->dev);

  struct buf *old_bp = bread(ip->dev, block);
  struct buf *new_bp = bread(ip->dev, new_block);
