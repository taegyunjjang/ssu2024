...
def main(args):
    ...
    """ split data """
    train_names, val_names = train_test_split(train_names, test_size=0.2, random_state=seed)
    
    """ training dataset loader """
    train_dataset = Dataset(root, train_names, train=True, transform=transforms)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                                            num_workers=os.cpu_count())
    
    """ validation datset loader """
    val_dataset = Dataset(root, val_names, train=False, transform=transforms)
    ...
    """ test datset loader """
    test_dataset = Dataset(root, test_names, train=False, transform=transforms)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size // 2, shuffle=False,
                                            num_workers=os.cpu_count()) 
    
    print(f'NUMBER OF TRAINING DATA SAMPLES: {len(train_dataset)}')
    print(f'NUMBER OF VALIDATION DATA SAMPLES: {len(val_dataset)}')
    print(f'NUMBER OF TEST DATA SAMPLES: {len(test_dataset)}')
    ...
    print(f'Test_Loss:{test_loss:07.3}')
